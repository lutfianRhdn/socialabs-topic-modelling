...
class Llm:
    ...
    def getContextByTopics(words, keyword):
        ...
        format_str = f"""dengan format JSON dengan 1 topik untuk 1 kalimat utama Berikut ini adalah format JSON-nya:
        [
            {{
                "kalimat": "..."
            }}
        ...
        # Combine components into the prompt
        prompt = f"Anda adalah {role} yang {action} {step} {context}. Buatkan {format_str}"
        ...
        try:
            ...
            # Mengambil item pertama dari res_json
            item = res_json[0]
            context_value = item['kalimat']
